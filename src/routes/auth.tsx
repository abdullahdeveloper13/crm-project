        ...
        <div className="rounded-2xl border border-border bg-white p-8 shadow-card">
          <div className="mb-6 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {mode === "signup" ? "Start closing more deals with NovaCRM AI." : mode === "forgot" ? "We'll email you a reset link." : "Sign in to your workspace."}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode !== "forgot" && (
              <Field label="Password" type="password" required minLength={6} value={password} onChange={setPassword} placeholder="••••••••" />
            )}
            <button
              type="submit"
              disabled={loading}
            ...
            {mode === "signin" && (
              <>
                ...
                <div className="text-muted-foreground">
                  New here?{" "}
                  <button onClick={() => setMode("signup")} className="font-medium text-primary hover:underline">Create an account</button>
                </div>
              </>
            )}
            {mode === "signup" && (
...
function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden>
      ...
      <path fill="#FBBC05" d="M5.84 14.1A6.98 6.98 0 0 1 5.47 12c0-.73.13-1.44.36-2.1V7.06H2.18A11 11 0 0 0 1 12c0 1.77.43 3.45 1.18 4.94l3.66-2.84z"/>
      <path fill="#EA4335" d="M12 5.38c1.61 0 3.06.55 4.2 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84C6.71 7.31 9.14 5.38 12 5.38z"/>
    </svg>
  );
}
